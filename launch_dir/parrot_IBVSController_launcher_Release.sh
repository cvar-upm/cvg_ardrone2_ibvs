...
gnome-terminal	 		\
	...
	--tab --title "IBVSController"	--command "bash -c \"
						roslaunch ./IBVS2013_launchfiles/parrot_IBVSController.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${IBVS_STACK}; 
						exec bash\""  \
	--tab --title "EKF"	--command "bash -c \"
	roslaunch ./IBVS2013_launchfiles/parrotStateEstimator.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${IBVS_STACK};
						exec bash\""	\
	--tab --title "IBVSCntInterf"	--command "bash -c \"
						roslaunch ./IBVS2013_launchfiles/parrotIBVSBrainInterface.launch --wait drone_id_namespace:=drone$NUMID_DRONE drone_id_int:=$NUMID_DRONE my_stack_directory:=${IBVS_STACK}; 
						exec bash\""	&
