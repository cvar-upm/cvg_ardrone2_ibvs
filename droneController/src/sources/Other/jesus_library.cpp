...
cvg_int jesus_library::fmod(cvg_int numerator, cvg_int denominator) {
	cvg_int quotient = numerator % denominator;
	if ( quotient < 0 )
...
cvg_double jesus_library::unitaryVectorFrom2Points( Vector &ur, Vector &p0, Vector &p1) { // from p0 to p1
...
cvg_double jesus_library::calculateVmax( Vector &r_ur, cvg_double vmax_xy, cvg_double vmax_z) {

	if ( r_ur.length() != 3 )
		return -1.0;

	cvg_double ur_x, ur_y, ur_z;
	jesus_library::getVectorComponents( r_ur, ur_x, ur_y, ur_z);
	cvg_double ur_xy = sqrt( ur_x*ur_x + ur_y*ur_y );
	ur_z = fabs(ur_z);

	cvg_double v1 = vmax_xy/ur_xy;
	cvg_double v2 = vmax_z/ur_z;

	return (v1 < v2) ? v1 : v2;
}

void jesus_library::mostrarVector( cvgString nombreVector, Vector &u) {
